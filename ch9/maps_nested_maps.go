...
func getNameCounts(names []string) map[rune]map[string]int {
	...
	for _, name := range names {
		runeName := rune(name[0])
		if _, ok := nameCounts[runeName]; ok {
			if _, ok := nameCounts[runeName][name]; ok {
				nameCounts[runeName][name] += 1
			} else {
				nameCounts[runeName][name] = 1
			}
		} else {
			nameCounts[runeName] = map[string]int{}
			nameCounts[runeName][name] = 1
		}
	}
	return nameCounts
...
func test(names []string, initial rune, name string) {
	...
	fmt.Printf("Generating counts for %v names...", len(names))
