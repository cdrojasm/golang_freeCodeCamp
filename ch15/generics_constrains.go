...
func chargeForLineItem[T lineItem](newItem T, oldItems []T, balance float64) ([]T, float64, error) {
	//?
	costNewItem := newItem.GetCost()
	newBalance := balance - costNewItem
	if balance < 0.0 {
		return oldItems, balance, errors.New("Insuficient funds")
	}
...
func test[T lineItem](newItem T, oldItems []T, balance float64) {
	...
	fmt.Printf("New balance is: %v. Total number of lines items is now %v\n", newBalance, len(newItems))
}
