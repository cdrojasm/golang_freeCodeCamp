...
func sendSMSToCouple(msgToCustomer, mstToSpouse string) (float64, error) {
	cost_0, err := sendSMS(msgToCustomer)
	if err != nil {
		return 0.0, err
	}
	cost_1, err := sendSMS(mstToSpouse)
	if err != nil {
		return 0.0, err
	}
	return cost_0 + cost_1, nil
}
